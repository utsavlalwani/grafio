...
. Run `:play news` command, and click and run the Cypher statement to insert the dataset
...
----
include::src/main/java/news/spring/data/neo4j/domain/Movie.java[tags=news]
...
----
include::src/main/java/news/spring/data/neo4j/repositories/MovieRepository.java[tags=repository]
...
The other two endpoints for finding multiple news by title and loading a single news are provided out of the box by the https://projects.spring.io/spring-data-rest/[Spring-Data-Rest project], which exposes our `MovieRepository` as REST endpoints.

The rendering of the news objects (and related entities) happens automatically via Jackson mapping.
...
----
// JSON object for single news with cast
curl http://localhost:8080/news?title=The%20Matrix

// list of JSON objects for news search results
curl http://localhost:8080/news?title=*matrix*
